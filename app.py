...
def bar_chart(df_in, title, y_col, y_label, is_currency=False):
    ...
    fig.update_layout(
        ...
        paper_bgcolor="white",
        height=650,
        margin=dict(t=120,b=200),  # slightly more top margin for taller blue header
    ...
    fig.add_shape(
        ...
        x0=0, x1=1,
        y0=1.00, y1=1.16,  # taller rectangle
        fillcolor=HEADER_BLUE,
        line_width=0
    )
    # ===== Header text centered vertically =====
    fig.add_annotation(
        x=0.5, y=1.08,  # vertically centered in taller rectangle
        xref="paper", yref="paper",
        text=f"<b>{title}</b>",
        showarrow=False,
