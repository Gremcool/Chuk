...
def bar_chart(df_in, title, y_col, y_label, is_currency=False):
    ...
    fig.update_layout(
        ...
        paper_bgcolor="white",
        height=650,
        margin=dict(t=130, b=200),  # slightly more top margin
        xaxis_title="Equipment",
        yaxis_title=y_label
    )
    # ===== Taller blue header rectangle =====
    fig.add_shape(
        type="rect",
        xref="paper", yref="paper",
        x0=0, x1=1,
        y0=1.00, y1=1.18,  # taller rectangle
        fillcolor=HEADER_BLUE,
        line_width=0
    )
    # ===== Centered header text vertically =====
    fig.add_annotation(
        x=0.5, y=1.09,  # center vertically in rectangle
        xref="paper", yref="paper",
        text=f"<b>{title}</b>",
        showarrow=False,
        font=dict(color="white", size=16),
        xanchor="center",
        yanchor="middle"  # key to vertically center the text
    )
